...
try {
  ...
  // Don't worry - we'll get by without it
}
...
module.exports = angular.module('or2.resource')
  .factory('resource.local', [
    ...
    'resource.query',
    '$localForage',
    function(socket, $q, $rootScope, $timeout, $window, ServerResourceFactory, QueryFactory, $localForage) {
      ...
      function LocalResourceFactory(url, rootKey, rootKeyPlural) {
        ...
        function doPersist() {
          ...
          switch(persistMode) {
          case 'FULL':
            data = _.map(_resources, function(res) {
              return {
                obj: res.$toObject(),
          ...
          case 'MIN':
            _.each(_resources, function(res) {
              if (_reqs[res._id]) {
                data.push({
                  obj: res.$toObject(),
                  lastreq: _lastreqs[res._id]
