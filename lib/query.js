...
function qryIsSimple(qry) {
  ...
  } else if (_.isObject(qry)) {
    _.each(qry, function(val, key) {
...
function extendQuery(qry1, qry2) {
  ...
  var _qry = _.clone(qry1, true);
  _.forEach(['limit', 'skip', 'sort'], function(prop) {
    if (qry2[prop]) {
      _qry[prop] = qry2[prop];
    }
  });

  if (!_.isEmpty(qry2.find)) {
    // Want to or together - but is the toplevel already an or? (and only an or)
    var existingOr = false;
    if (_qry.find.$or) {
      var valid = true;
      _.forEach(_qry.find, function(val, key) {
        if (key !== '$or') {
          valid = false;
          return false;
        }
...
module.exports = angular.module('or2.resource')
  .factory('resource.query', [
    '$rootScope',
    '$q',
    '$timeout',
    '$injector',
    'resource.chain',
    function($rootScope, $q, $timeout, $injector, Chain) {
      ...
      function QueryFactory(url, rootKey, rootKeyPlural, db) {
        ...
        function refreshQueries() {
          var resultsSet = _.chain(_serverQueries).values().flatten(true).uniq().value();
          _.forEach(resultsSet, function(results) {
            results.$refresh();
          });
        }
        ...
        function LocalQueryList(qry, limit, Resource, toRes) {
          ...
          function syncFromServer() {
            ...
            if (fallback) {
              ...
              results.length = 0;
              _.each(serverResults, function(res) {
                results.push(res);
              });
          ...
          function query(_qry) {
            ...
            cur.exec(function(err, docs) {
              $rootScope.$apply(function() {
                ...
                var tmpResults = _.map(docs, toRes);

                // Do we need to do a transform?
                var rprom;
                if (_.isFunction(results.transform)) {
                ...
                return rprom.then(function(transformed) {
                  ...
                  results.length = 0;
                  _.each(transformed, function(res) {
                    results.push(res);
                  });
        ...
        LocalQueryList.refresh = function refresh() {
          ...
          function doRefresh() {
            _.each(_localQueries, function(res) {
              res.$refresh();
            });
            refreshTimer = null;
          }
        ...
        function ServerQueryList(qry, limit, Resource) {
          ...
          function newData(_qryId, response, force) {
            ...
            var getIds = ids;
            if (force) {
              var oldIds = _.pluck(results, '_id');
              getIds = _.union(ids, oldIds);
            }
          ...
          function maybeNewQryId(_qryId) {
            if (qryId !== _qryId) {
              ...
              if (qryId) {
                ...
                qryList = _serverQueries[qryId] = _.without(_serverQueries[qryId], results);
